...
TEST(test_f1_detection_with_parser) {
    ...
    /* With parser initialized, we might need multiple reads to accumulate the sequence */
    /* Try reading up to 3 times to get the complete event */
    int read_attempts = 0;
    while (result == LLE_SUCCESS && event.type == LLE_INPUT_TYPE_TIMEOUT && read_attempts < 3) {
...
TEST(test_f5_detection_with_parser) {
    ...
    int read_attempts = 0;
    while (result == LLE_SUCCESS && event.type == LLE_INPUT_TYPE_TIMEOUT && read_attempts < 5) {
        result = lle_unix_interface_read_event(interface, &event, 100);
        read_attempts++;
    }
