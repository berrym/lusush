...
static lle_editor_t* create_editor_with_content(const char *content, lusush_memory_pool_t *pool) {
    ...
    /* Insert content */
    if (content && *content) {
        result = lle_buffer_insert_text(editor->buffer, 0, content, strlen(content));
...
static void get_cursor_position(lle_editor_t *editor, size_t *byte_offset, 
                                size_t *codepoint_index, size_t *grapheme_index) {
    lle_cursor_position_t pos;
    lle_cursor_manager_get_position(editor->cursor_manager, &pos);
    
    if (byte_offset) *byte_offset = pos.byte_offset;
    if (codepoint_index) *codepoint_index = pos.codepoint_index;
    if (grapheme_index) *grapheme_index = pos.grapheme_index;
}
...
static void test_forward_char_ascii(lusush_memory_pool_t *pool) {
    ...
    get_cursor_position(editor, &byte_off, &cp_idx, &gr_idx);
    TEST_ASSERT(byte_off == 1 && gr_idx == 1, "Position after 1 forward incorrect");
    ...
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, &cp_idx, &gr_idx);
    TEST_ASSERT(byte_off == 4 && gr_idx == 4, "Position after 4 forwards incorrect");
...
static void test_forward_char_utf8_2byte(lusush_memory_pool_t *pool) {
    ...
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 2 && gr_idx == 2, "Position after 2 forwards incorrect");
    
    /* Move forward 3: should be at 'é' start (byte 3) */
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 3 && gr_idx == 3, "Position after 3 forwards incorrect");
    
    /* Move forward 4: should be past 'é' (byte 5, end of string) */
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 5 && gr_idx == 4, "Position after 'é' incorrect (2-byte char)");
...
static void test_forward_char_utf8_3byte(lusush_memory_pool_t *pool) {
    ...
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 3 && gr_idx == 1, "Position after first CJK char incorrect");
    
    /* Move forward 1 more: should skip another 3 bytes */
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 6 && gr_idx == 2, "Position after second CJK char incorrect");
...
static void test_forward_char_utf8_4byte(lusush_memory_pool_t *pool) {
    ...
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 4 && gr_idx == 1, "Position after first emoji incorrect");
    
    /* Move forward 1 more emoji: should skip another 4 bytes */
    lle_forward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 8 && gr_idx == 2, "Position after second emoji incorrect");
...
static void test_backward_char_utf8(lusush_memory_pool_t *pool) {
    ...
    lle_backward_char(editor);
    get_cursor_position(editor, &byte_off, NULL, &gr_idx);
    TEST_ASSERT(byte_off == 0 && gr_idx == 0, "Should not move before beginning");
...
static void test_forward_word_utf8(lusush_memory_pool_t *pool) {
    ...
    lle_forward_word(editor);
    get_cursor_position(editor, &byte_off, NULL, NULL);
    TEST_ASSERT(byte_off == 12, "Should be at end of '中文' (6 bytes after space)");
