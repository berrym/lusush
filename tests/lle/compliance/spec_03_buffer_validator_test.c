...
static lle_buffer_t* create_test_buffer(const char *content) {
    ...
    buffer->capacity = content_len + 1 > LLE_BUFFER_MIN_CAPACITY ? 
                       content_len + 1 : LLE_BUFFER_MIN_CAPACITY;
...
static void test_buffer_validator_structure() {
    ...
    ASSERT_TRUE(sizeof(val.utf8_validation_enabled) > 0, "utf8_validation_enabled exists");
    ASSERT_TRUE(sizeof(val.validation_count) > 0, "validation_count exists");
    ASSERT_TRUE(sizeof(val.last_validation_result) > 0, "last_validation_result exists");
...
static void test_buffer_validator_init() {
    ...
    ASSERT_TRUE(validator->utf8_validation_enabled, "UTF-8 validation enabled by default");
    ASSERT_TRUE(validator->line_structure_validation, "Line validation enabled by default");
    ASSERT_TRUE(validator->cursor_validation_enabled, "Cursor validation enabled by default");
    ASSERT_TRUE(validator->bounds_checking_enabled, "Bounds checking enabled by default");
    ASSERT_EQ(validator->validation_count, 0, "Validation count initially zero");
    ASSERT_EQ(validator->validation_failures, 0, "Validation failures initially zero");
...
static void test_detect_buffer_overflow() {
    ...
    ASSERT_TRUE(validator->validation_failures > 0, "Validation failure recorded");
...
static void test_error_handling() {
    ...
    ASSERT_TRUE(result == LLE_ERROR_INVALID_PARAMETER, "Validate rejects NULL buffer");
