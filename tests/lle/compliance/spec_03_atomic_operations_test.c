...
static void test_buffer_insert() {
    ...
    ASSERT_STR_EQ(buffer->data, "Hello World", "Buffer content after second insert");
...
static void test_change_tracker_init() {
    ...
    lle_result_t result = lle_change_tracker_init(&tracker, test_memory_pool, 100);
    ASSERT_SUCCESS(result, "Tracker init failed");
    ASSERT_TRUE(tracker != NULL, "Tracker is NULL");
    
    ASSERT_TRUE(!lle_change_tracker_can_undo(tracker), "Should not be able to undo initially");
    ASSERT_TRUE(!lle_change_tracker_can_redo(tracker), "Should not be able to redo initially");
