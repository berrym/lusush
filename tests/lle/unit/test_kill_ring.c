...
TEST(create_destroy) {
    ...
    ASSERT_EQ(capacity, LLE_KILL_RING_DEFAULT_SIZE, "Capacity not default size");
...
TEST(null_pointer_checks) {
    ...
    ASSERT(result == LLE_ERROR_NULL_POINTER, "Create accepted NULL ring pointer");
...
TEST(yank_from_empty_ring) {
    ...
    const char *text;
    result = lle_kill_ring_get_current(ring, &text);
    ASSERT(result == LLE_ERROR_QUEUE_EMPTY, "Yank from empty didn't return QUEUE_EMPTY error");
...
TEST(yank_pop_without_yank_fails) {
    ...
    ASSERT(result == LLE_ERROR_INVALID_STATE, "Yank-pop without yank didn't fail");
...
TEST(readline_compat_successive_kill_line) {
    ...
    const char *text;
    result = lle_kill_ring_get_current(ring, &text);
    ASSERT_STR_EQ(text, "first line\nsecond line", "Successive C-k text incorrect");
...
TEST(large_text_handling) {
    ...
    ASSERT(strlen(text) == sizeof(large_text) - 1, "Large text length incorrect");
