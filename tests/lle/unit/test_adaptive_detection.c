...
static void test_signature_database(void) {
    ...
    size_t count = 0;
    const lle_terminal_signature_t *sigs = lle_get_terminal_signature_database(&count);
    ...
    for (size_t i = 0; i < count; i++) {
        if (strcmp(sigs[i].name, "zed") == 0) found_zed = true;
        if (strcmp(sigs[i].name, "vscode") == 0) found_vscode = true;
        if (strcmp(sigs[i].name, "iterm2") == 0) found_iterm2 = true;
        if (strcmp(sigs[i].name, "tmux") == 0) found_tmux = true;
    }
...
static void test_basic_detection(void) {
    ...
    if (result) {
        TEST_ASSERT(result->detection_time_us > 0, "Detection time is recorded");
        ...
        TEST_ASSERT(result->recommended_mode >= LLE_ADAPTIVE_MODE_NONE &&
                   result->recommended_mode <= LLE_ADAPTIVE_MODE_MULTIPLEXED,
...
static void test_utility_functions(void) {
    ...
    TEST_ASSERT(lle_adaptive_mode_to_string(LLE_ADAPTIVE_MODE_MULTIPLEXED) != NULL,
