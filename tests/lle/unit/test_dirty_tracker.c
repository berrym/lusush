...
TEST(tracker_mark_full_null) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER,
              "Mark full with NULL should return error");
}
...
TEST(tracker_mark_region_null) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER,
              "Mark region with NULL should return error");
}
...
TEST(tracker_mark_multiple_regions) {
    ...
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 50),
                "Region 50 should be dirty");
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 100),
                "Region 100 should be dirty");
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 200),
                "Region 200 should be dirty");
...
TEST(tracker_mark_range_null) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER,
              "Mark range with NULL should return error");
}
...
TEST(tracker_mark_range_success) {
    ...
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 125),
                "Middle of range should be dirty");
...
TEST(tracker_clear_null) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER,
              "Clear with NULL should return error");
}
...
TEST(tracker_clear_success) {
    ...
    ASSERT_FALSE(lle_dirty_tracker_is_region_dirty(tracker, 100),
                 "Region should be clean after clear");
    ASSERT_FALSE(lle_dirty_tracker_is_region_dirty(tracker, 200),
                 "Region should be clean after clear");
...
TEST(tracker_clear_after_full) {
    ...
    ASSERT_FALSE(lle_dirty_tracker_needs_full_redraw(tracker),
                 "Should not need full redraw after clear");
...
TEST(tracker_mark_clear_cycle) {
    ...
    lle_dirty_tracker_clear(tracker);
    ASSERT_FALSE(lle_dirty_tracker_is_region_dirty(tracker, 100),
                 "Region should be clean");

    lle_dirty_tracker_mark_region(tracker, 100);
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 100),
                "Region should be dirty again");
...
TEST(tracker_overlapping_ranges) {
    ...
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 100),
                "First range start should be dirty");
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 145),
                "Overlap region should be dirty");
    ASSERT_TRUE(lle_dirty_tracker_is_region_dirty(tracker, 165),
                "Second range end should be dirty");
