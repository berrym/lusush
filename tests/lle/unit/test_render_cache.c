...
TEST(cache_store_null_cache) {
    ...
    lle_result_t result = lle_display_cache_store(NULL, 123, data, strlen(data));
    
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER, "NULL cache should return error");
}
...
TEST(cache_store_success) {
    ...
    const char *data = "test data";
    lle_result_t result = lle_display_cache_store(cache, 123, data, strlen(data));
...
TEST(cache_lookup_miss) {
    ...
    ASSERT_EQ(result, LLE_ERROR_CACHE_MISS, "Non-existent key should return CACHE_MISS");
...
TEST(cache_store_and_lookup_cycle) {
    ...
    for (uint64_t i = 0; i < 3; i++) {
        ...
        lle_result_t result = lle_display_cache_lookup(cache, i, &lookup_data, &lookup_size);
...
TEST(cache_invalidate_existing_entry) {
    ...
    ASSERT_EQ(result, LLE_ERROR_CACHE_MISS, "Invalidated entry should not be found");
...
TEST(cache_invalidate_all_success) {
    ...
    for (uint64_t i = 0; i < 5; i++) {
        ...
        ASSERT_EQ(result, LLE_ERROR_CACHE_MISS, "All entries should be invalidated");
    }
...
TEST(render_cache_cleanup_null) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER, "NULL cleanup should return error");
}
