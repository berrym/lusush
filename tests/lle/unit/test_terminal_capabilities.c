...
TEST(test_color_depth_valid) {
    ...
    assert(caps->detected_color_depth == 0 ||
...
TEST(test_text_attributes_detected) {
    ...
    assert(caps->supports_underline == true || caps->supports_underline == false);
    assert(caps->supports_strikethrough == true || caps->supports_strikethrough == false);
...
TEST(test_advanced_features_detected) {
    ...
    assert(caps->supports_mouse_reporting == true || caps->supports_mouse_reporting == false);
    assert(caps->supports_bracketed_paste == true || caps->supports_bracketed_paste == false);
    assert(caps->supports_focus_events == true || caps->supports_focus_events == false);
    assert(caps->supports_synchronized_output == true || caps->supports_synchronized_output == false);
...
TEST(test_performance_characteristics) {
    ...
    assert(caps->supports_fast_updates == true || caps->supports_fast_updates == false);
