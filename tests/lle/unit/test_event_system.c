...
TEST(event_system_init_success) {
    ...
    ASSERT_EQ(system->sequence_counter, 1, "Sequence counter should start at 1");
...
TEST(event_system_init_null_system) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER, "Should reject NULL system pointer");
}
...
TEST(event_create_success_no_data) {
    ...
    lle_result_t result = lle_event_create(system, LLE_EVENT_KEY_PRESS, NULL, 0, &event);
...
TEST(event_create_success_with_data) {
    ...
    lle_event_t *event = NULL;
    lle_result_t result = lle_event_create(system, LLE_EVENT_BUFFER_CHANGED, 
                                          (void*)test_data, strlen(test_data) + 1, &event);
    
    ASSERT_EQ(result, LLE_SUCCESS, "Event creation should succeed");
    ASSERT_NOT_NULL(event, "Event should be allocated");
    ASSERT_NOT_NULL(event->data, "Data should be copied");
    ASSERT_EQ(event->data_size, strlen(test_data) + 1, "Data size should match");
...
TEST(event_create_null_system) {
    lle_event_t *event = NULL;
    lle_result_t result = lle_event_create(NULL, LLE_EVENT_KEY_PRESS, NULL, 0, &event);
...
TEST(event_create_null_event_ptr) {
    ...
    lle_result_t result = lle_event_create(system, LLE_EVENT_KEY_PRESS, NULL, 0, NULL);
    
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER, "Should reject NULL event pointer");
...
TEST(event_clone_success) {
    ...
    lle_event_create(system, LLE_EVENT_KEY_SEQUENCE, 
                    (void*)test_data, strlen(test_data) + 1, &original);
    ...
    ASSERT_EQ(strcmp(clone->data, original->data), 0, "Data content should match");
...
TEST(event_queue_empty_check) {
    ...
    ASSERT_TRUE(lle_event_queue_empty(system), "Queue should be empty initially");
    
    lle_event_t *event = NULL;
    lle_event_create(system, LLE_EVENT_KEY_PRESS, NULL, 0, &event);
    lle_event_enqueue(system, event);
    
    ASSERT_FALSE(lle_event_queue_empty(system), "Queue should not be empty after enqueue");
...
TEST(handler_register_success) {
    ...
    lle_result_t result = lle_event_handler_register(system, LLE_EVENT_KEY_PRESS,
                                                     test_event_handler, NULL, "test_handler");
...
TEST(handler_register_multiple_types) {
    ...
    lle_event_handler_register(system, LLE_EVENT_KEY_PRESS, test_event_handler, NULL, "handler1");
    lle_event_handler_register(system, LLE_EVENT_CURSOR_MOVED, test_event_handler, NULL, "handler2");
    lle_event_handler_register(system, LLE_EVENT_BUFFER_CHANGED, test_event_handler, NULL, "handler3");
...
TEST(handler_register_null_system) {
    lle_result_t result = lle_event_handler_register(NULL, LLE_EVENT_KEY_PRESS,
                                                     test_event_handler, NULL, "test");
...
TEST(handler_register_null_function) {
    ...
    ASSERT_EQ(result, LLE_ERROR_INVALID_PARAMETER, "Should reject NULL handler function");
...
TEST(handler_dispatch_success) {
    ...
    lle_event_handler_register(system, LLE_EVENT_KEY_PRESS, test_event_handler, NULL, "test");
    ...
    ASSERT_EQ(last_handled_event, event, "Handler should receive correct event");
...
TEST(handler_dispatch_no_matching_handler) {
    ...
    ASSERT_EQ(result, LLE_SUCCESS, "Dispatch should succeed even with no handlers");
...
TEST(handler_dispatch_user_data) {
    ...
    int test_data = 42;
    lle_event_handler_register(system, LLE_EVENT_KEY_PRESS, test_event_handler, &test_data, "test");
    ...
    ASSERT_EQ(last_handler_user_data, &test_data, "Handler should receive user data");
    ASSERT_EQ(*(int*)last_handler_user_data, 42, "User data content should be correct");
...
TEST(handler_unregister_by_name) {
    ...
    lle_event_handler_register(system, LLE_EVENT_KEY_PRESS, test_event_handler, NULL, "test_handler");
    ASSERT_EQ(system->handler_count, 1, "Should have 1 handler");
    
    lle_result_t result = lle_event_handler_unregister(system, LLE_EVENT_KEY_PRESS, "test_handler");
...
TEST(handler_unregister_not_found) {
    ...
    lle_result_t result = lle_event_handler_unregister(system, LLE_EVENT_KEY_PRESS, "nonexistent");
...
TEST(statistics_events_created) {
    ...
    ASSERT_EQ(system->events_created, initial + 2, "Should track created events");
...
TEST(statistics_events_dispatched) {
    ...
    ASSERT_EQ(system->events_dispatched, initial + 2, "Should track dispatched events");
...
TEST(event_type_name_unknown) {
    ...
    ASSERT_EQ(strcmp(name, "UNKNOWN"), 0, "Should return 'UNKNOWN' for invalid type");
}
