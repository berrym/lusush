...
TEST(extract_syntax_colors_null_theme) {
    ...
    lle_result_t result =
        lle_extract_syntax_colors_from_theme(NULL, &table, mock_pool);

    ASSERT_EQ(result, LLE_ERROR_NULL_POINTER,
              "Null theme should return NULL_POINTER error");
}
...
TEST(extract_syntax_colors_null_output) {
    ...
    lle_result_t result =
        lle_extract_syntax_colors_from_theme(&theme, NULL, mock_pool);

    ASSERT_EQ(result, LLE_ERROR_NULL_POINTER,
              "Null output pointer should return NULL_POINTER error");
}
...
TEST(extract_syntax_colors_success) {
    ...
    lle_result_t result =
        lle_extract_syntax_colors_from_theme(&theme, &table, mock_pool);
    ...
    ASSERT_EQ(table->theme_context, (void *)&theme,
              "Theme context should be stored");
}
...
TEST(extract_syntax_colors_parsing) {
     ...
     * We verify extraction worked rather than exact color values since
     * 256-color to RGB mapping depends on the terminal's color palette. */
...
TEST(extract_cursor_colors_null_theme) {
    ...
    lle_result_t result =
        lle_extract_cursor_colors_from_theme(NULL, &colors, mock_pool);
...
TEST(extract_cursor_colors_null_output) {
    ...
    lle_result_t result =
        lle_extract_cursor_colors_from_theme(&theme, NULL, mock_pool);
...
TEST(extract_cursor_colors_success) {
    ...
    lle_result_t result =
        lle_extract_cursor_colors_from_theme(&theme, &colors, mock_pool);
    ...
    ASSERT_NEQ(colors->cursor_background_color, 0,
               "Cursor background color should be set");
}
...
TEST(integrate_theme_null_integration) {
    ...
    ASSERT_EQ(result, LLE_ERROR_NULL_POINTER,
              "Null integration should return NULL_POINTER error");
}
...
TEST(integrate_theme_null_theme_allowed) {
    ...
    lle_result_t result =
        lle_display_integrate_theme_system(&integration, NULL);

    ASSERT_EQ(result, LLE_SUCCESS,
              "Null theme should be allowed (use defaults)");
...
TEST(integrate_theme_success) {
    ...
    ASSERT_EQ(integration.theme_system, (void *)&theme,
              "Theme should be stored");
    ASSERT_NOT_NULL(controller.theme_colors,
                    "Syntax colors should be extracted");
    ASSERT_NOT_NULL(controller.cursor_theme_colors,
                    "Cursor colors should be extracted");
}
...
TEST(theme_changed_null_theme_name) {
    ...
    ASSERT_EQ(result, LLE_ERROR_NULL_POINTER,
              "Null theme name should return NULL_POINTER error");
}
...
TEST(extract_colors_256color_mode) {
    ...
    ASSERT_NEQ(table->keyword_color, 0xFFFFFF,
               "Primary should not be default white");
    ASSERT_NEQ(table->string_color, 0xFFFFFF,
               "Success should not be default white");
}
...
TEST(extract_colors_basic_ansi) {
    ...
    ASSERT_EQ(table->string_color, 0x00FF00,
              "Green ANSI code should parse to 0x00FF00");
    ASSERT_EQ(table->number_color, 0x0000FF,
              "Blue ANSI code should parse to 0x0000FF");
    ASSERT_EQ(table->default_color, 0xFFFFFF,
              "White ANSI code should parse to 0xFFFFFF");
}
...
TEST(extract_colors_empty_codes) {
    ...
    ASSERT_EQ(table->keyword_color, 0xFFFFFF,
              "Empty code should default to white");
    ASSERT_EQ(table->string_color, 0xFFFFFF,
              "Empty code should default to white");
    ASSERT_EQ(table->comment_color, 0xFFFFFF,
              "Empty code should default to white");
}
