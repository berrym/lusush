...
static lle_terminal_capabilities_t *mock_terminal = (lle_terminal_capabilities_t *)&mock_terminal_dummy;
...
void test_init_invalid_params(void) {
    ...
    ASSERT(result == LLE_ERROR_INVALID_PARAMETER, "Init with NULL detector should fail");
    
    result = lle_key_detector_init(&detector, NULL, mock_pool);
    ASSERT(result == LLE_ERROR_INVALID_PARAMETER, "Init with NULL terminal should fail");
...
void test_detect_f1_key(void) {
    ...
    result = lle_key_detector_process_sequence(detector, f1_sequence, 3, &key_info);
...
void test_detect_cursor_up(void) {
    ...
    result = lle_key_detector_process_sequence(detector, up_sequence, 3, &key_info);
    ASSERT(result == LLE_SUCCESS, "Process should succeed");
    ASSERT(key_info != NULL, "Should detect key");
...
void test_detect_ctrl_c(void) {
    ...
    ASSERT(key_info->modifiers == LLE_KEY_MOD_CTRL, "Should have Ctrl modifier");
...
void test_detect_shift_up(void) {
    ...
    result = lle_key_detector_process_sequence(detector, shift_up, 6, &key_info);
    ASSERT(result == LLE_SUCCESS, "Process should succeed");
    ASSERT(key_info != NULL, "Should detect key");
    ASSERT(key_info->type == LLE_KEY_TYPE_CURSOR, "Should be cursor key");
    ASSERT(key_info->modifiers == LLE_KEY_MOD_SHIFT, "Should have Shift modifier");
...
void test_detect_ctrl_right(void) {
    ...
    result = lle_key_detector_process_sequence(detector, ctrl_right, 6, &key_info);
    ASSERT(result == LLE_SUCCESS, "Process should succeed");
    ASSERT(key_info != NULL, "Should detect key");
    ASSERT(key_info->type == LLE_KEY_TYPE_CURSOR, "Should be cursor key");
    ASSERT(key_info->modifiers == LLE_KEY_MOD_CTRL, "Should have Ctrl modifier");
...
void test_detect_delete_key(void) {
    ...
    result = lle_key_detector_process_sequence(detector, delete_key, 4, &key_info);
    ASSERT(result == LLE_SUCCESS, "Process should succeed");
    ASSERT(key_info != NULL, "Should detect key");
    ASSERT(key_info->type == LLE_KEY_TYPE_EDITING, "Should be editing key");
    ASSERT(strcmp(key_info->key_name, "Delete") == 0, "Key name should be Delete");
...
void test_get_statistics(void) {
    ...
    result = lle_key_detector_get_stats(detector, &detected, &resolved, &timeouts);
