...
static int test_complete_lifecycle(void) {
    ...
    char api_token[512];
    strcpy(api_token, "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0");
    ...
    switch (result) {
        ...
        case LLE_ERROR_PERMISSION_DENIED:
            printf("  ⚠ Secure mode denied (no CAP_IPC_LOCK) - continuing with degraded security\n");
            break;
        case LLE_ERROR_OUT_OF_MEMORY:
            printf("  ⚠ RLIMIT_MEMLOCK exceeded - continuing with degraded security\n");
            break;
        case LLE_ERROR_RESOURCE_UNAVAILABLE:
            printf("  ⚠ Resource unavailable - continuing with degraded security\n");
            break;
        case LLE_ERROR_FEATURE_NOT_AVAILABLE:
            printf("  ⚠ mlock not available on this platform - continuing with degraded security\n");
            break;
        default:
...
static int test_multiple_regions(void) {
    ...
    lle_result_t r3 = lle_memory_enable_secure_mode(session_token, sizeof(session_token));
    ...
    // Disable secure mode
    if (r1 == LLE_SUCCESS) lle_memory_disable_secure_mode(password, sizeof(password));
    if (r2 == LLE_SUCCESS) lle_memory_disable_secure_mode(api_key, sizeof(api_key));
    if (r3 == LLE_SUCCESS) lle_memory_disable_secure_mode(session_token, sizeof(session_token));
