...
void test_forensic_apply_to_entry(void) {
    ...
    lle_result_t result = lle_history_entry_create(&entry, "test command", NULL);
...
void test_forensic_timing(void) {
    ...
    ASSERT_TRUE(entry->end_time_ns > entry->start_time_ns, "End time should be after start");
...
void test_forensic_usage_tracking(void) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ASSERT_EQ(entry->usage_count, (uint32_t)i, "Usage count should increment");
    }
...
void test_dedup_statistics(void) {
    ...
    ASSERT_TRUE(stats.duplicates_detected > 0, "Should have detected duplicates");
...
void test_multiline_structure_analysis(void) {
    ...
    lle_result_t result = lle_history_detect_multiline_structure(function_def, &info);
...
void test_multiline_reconstruction(void) {
    ...
    ASSERT_TRUE(strchr(buffer, '\n') == NULL, "Flattened should not have newlines");
...
void test_multiline_line_analysis(void) {
    ...
    lle_result_t result = lle_history_analyze_multiline_lines(multiline, &lines, &line_count);
    ASSERT_EQ(result, LLE_SUCCESS, "Analysis should succeed");
