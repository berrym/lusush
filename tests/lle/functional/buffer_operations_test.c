...
static void test_buffer_create_destroy() {
    ...
    ASSERT_TRUE(buffer->capacity >= LLE_BUFFER_MIN_CAPACITY, "Buffer has minimum capacity");
...
static void test_buffer_create_with_capacity() {
    ...
    lle_result_t result = lle_buffer_create(&buffer, test_pool, requested_capacity);
    
    ASSERT_SUCCESS(result, "Buffer creation succeeds");
    ASSERT_TRUE(buffer->capacity >= requested_capacity, "Buffer has requested capacity");
...
static void test_insert_utf8_text() {
    ...
    ASSERT_EQ(buffer->length, 12, "Buffer length correct (6 ASCII + 6 UTF-8 bytes)");
...
static void test_multiple_insertions() {
    ...
    ASSERT_STR_EQ(buffer->data, "abc", "Content correct after multiple insertions");
...
static void test_insert_delete_sequence() {
    ...
    ASSERT_STR_EQ(buffer->data, "Hello Earth", "Content correct after sequence");
...
static void test_buffer_growth() {
    ...
    lle_buffer_t *buffer = NULL;
    lle_result_t result = lle_buffer_create(&buffer, test_pool, LLE_BUFFER_MIN_CAPACITY);
    ASSERT_SUCCESS(result, "Buffer creation succeeds");
...
static void test_insert_out_of_bounds() {
    ...
    ASSERT_TRUE(result == LLE_ERROR_INVALID_RANGE, "Out of bounds insert rejected");
...
static void test_delete_out_of_bounds() {
    ...
    ASSERT_TRUE(result == LLE_ERROR_INVALID_RANGE, "Out of bounds delete rejected");
