 ...
 * Compile: gcc -o test_grapheme_nav test_grapheme_nav.c -I../../include -L../../builddir/src/lle -llle -L../../builddir/src/pool -llusush_pool
 */
...
int main(void) {
    ...
    for (size_t i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) {
        ...
        /* Try to sync cursor */
        result = lle_cursor_manager_move_to_byte_offset(cursor_mgr, buffer->cursor.byte_offset);
        if (result != LLE_SUCCESS) {
        ...
        printf("  %s\n\n", (buffer->grapheme_count == tests[i].expected_graphemes) ? "PASS" : "FAIL");
