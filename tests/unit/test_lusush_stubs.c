...
int ht_strstr_enum_next(ht_strstr_enum_t *e, const char **key, const char **value) {
    if (!e || !e->ht) return 0;
    ...
    if (key) *key = e->ht->keys[e->index];
    if (value) *value = e->ht->values[e->index];
    
    e->index++;
    return 1;
}

void ht_strstr_enum_destroy(ht_strstr_enum_t *e) {
    free(e);
}
