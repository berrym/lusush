                ...
                if (*next && *next != '\n') {
                    ...
                    if (dst > result && *(dst - 1) != ';' &&
                        *(dst - 1) != '{' && *(dst - 1) != '(' &&
                        ...
                        if (strncmp(next, "then", 4) == 0 ||
                            strncmp(next, "do", 2) == 0 ||
                            strncmp(next, "else", 4) == 0 ||
                            strncmp(next, "elif", 4) == 0 ||
                            strncmp(next, "fi", 2) == 0 ||
                            strncmp(next, "done", 4) == 0 ||
...
test_case_t test_cases[] = {
    ...
    {                         "for i in 1 2 3\ndo\n    echo $i\ndone",                                "for i in 1 2 3 do echo $i done",
     "Basic FOR loop"                                                                                                                                                      },
    {   "while [ $x -le 3 ]\ndo\n    echo $x\n    x=$((x + 1))\ndone",
     "while [ $x -le 3 ] do echo $x x=$((x + 1)) done", "WHILE loop with multiple commands"                                                                                },
    {                          "{\n    echo line1\n    echo line2\n}",                                     "{ echo line1 echo line2 }",
     "Brace group"                                                                                                                                                         },
    {                                          "echo hello \\\nworld",                                              "echo hello world",            "Backslash continuation"},
    {                                    "echo 'multi\nline\nstring'",                                  "echo 'multi\\nline\\nstring'",
