...
typedef struct {
    ...
    uint64_t recent_render_times[PROMPT_LAYER_METRICS_HISTORY_SIZE]; // Recent timing history
 ...
 * @note Supports all 6 built-in themes (dark, light, minimal, colorful, classic, corporate)
 */
 ...
 * @return PROMPT_LAYER_SUCCESS on success, error code on failure
 * 
 * @note Uses existing build_prompt() and theme_generate_primary_prompt() functions
