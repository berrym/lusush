...
const fuzzy_match_options_t FUZZY_MATCH_STRICT = {
    .case_sensitive = true,
...
const fuzzy_match_options_t FUZZY_MATCH_FAST = {
    .case_sensitive = false,
    .unicode_normalize = false,
    .use_damerau = false,
    .max_distance = 0
...
static int decode_to_codepoints(const char *str, codepoint_array_t *out,
    ...
    if (opts && opts->unicode_normalize) {
        size_t norm_len;
        if (lle_unicode_normalize_nfc(str, input_len, norm_buf, NORM_BUFFER_SIZE, &norm_len) == 0) {
            input = norm_buf;
            input_len = norm_len;
        }
...
int fuzzy_levenshtein_distance(const char *s1, const char *s2,
    ...
    if (!s1 && !s2) return 0;
    if (!s1) return strlen(s2);
    if (!s2) return strlen(s1);
    
    const fuzzy_match_options_t *opts = options ? options : &FUZZY_MATCH_DEFAULT;
...
static int damerau_levenshtein_codepoints(const codepoint_array_t *s1,
    ...
    /* Need full matrix for Damerau-Levenshtein (transpositions need d[i-2][j-2]) */
    int **d = malloc((len1 + 1) * sizeof(int *));
    if (!d) return len1 + len2;
...
int fuzzy_match_score(const char *s1, const char *s2,
     ...
     */
    int final_score = (levenshtein_score * 4 +
                       jaro_score * 3 +
                       prefix_score * 2 +
                       subseq_score * 1) / 10;
...
int fuzzy_match_best(const char *pattern,
    ...
    fuzzy_match_result_t *all_results = malloc(num_candidates * sizeof(fuzzy_match_result_t));
    if (!all_results) return 0;
