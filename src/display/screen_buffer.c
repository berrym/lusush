...
void screen_buffer_render(screen_buffer_t *buffer, const char *prompt_text,
    ...
    if (command_text) {
        ...
        // Keep cursor positions as ABSOLUTE screen coordinates for
        // screen_buffer_diff/apply_diff (they use ESC[row;colH which expects
        // absolute positions)
    }
...
void screen_buffer_diff(const screen_buffer_t *old_buffer,
    ...
    for (int row = 0;
         row < max_rows && diff->num_changes < SCREEN_BUFFER_MAX_ROWS * 2;
         row++) {
        ...
        const screen_line_t *old_line =
            row < old_buffer->num_rows ? &old_buffer->lines[row] : NULL;
        ...
        for (int col = 0;
             col < new_line->length || (old_line && col < old_line->length);
             col++) {
            const screen_cell_t *old_cell = (old_line && col < old_line->length)
                                                ? &old_line->cells[col]
                                                : NULL;
            const screen_cell_t *new_cell =
                (col < new_line->length) ? &new_line->cells[col] : NULL;
            ...
            if (!old_cell && new_cell) {
                ...
                if (old_cell->byte_len != new_cell->byte_len) {
                    ...
                    cells_differ =
                        (memcmp(old_cell->utf8_bytes, new_cell->utf8_bytes,
                                old_cell->byte_len) != 0);
                }
            }
        ...
        if (first_diff != -1) {
            ...
            for (int col = first_diff;
                 col <= last_diff && col < new_line->length; col++) {
                const screen_cell_t *cell = &new_line->cells[col];

                // Copy full UTF-8 sequence
                for (int b = 0; b < cell->byte_len &&
                                change->text_len < SCREEN_BUFFER_MAX_COLS - 1;
                     b++) {
            ...
            if (old_line && last_diff < old_line->length - 1 &&
                last_diff == new_line->length - 1) {
                screen_change_t *clear_change =
                    &diff->changes[diff->num_changes++];
