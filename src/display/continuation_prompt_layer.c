 ...
 */
static void calculate_command_hash(const char *command, char *hash_out, size_t hash_size) {
...
static bool cache_lookup(
    ...
    if (!layer || !command_hash || !out_prompt) return false;
...
static void cache_store(
    ...
    if (!layer || !command_hash || !prompt) return;
...
continuation_prompt_layer_t *continuation_prompt_layer_create(void) {
    continuation_prompt_layer_t *layer = 
        (continuation_prompt_layer_t *)calloc(1, sizeof(continuation_prompt_layer_t));
...
continuation_prompt_mode_t continuation_prompt_layer_get_mode(
...
continuation_prompt_error_t continuation_prompt_layer_get_prompt_for_line(
    ...
    if (layer->mode == CONTINUATION_PROMPT_MODE_SIMPLE) {
        ...
        if (elapsed > layer->max_time_ns) layer->max_time_ns = elapsed;
        if (elapsed < layer->min_time_ns) layer->min_time_ns = elapsed;
    ...
    if (cache_lookup(layer, command_hash, line_number, out_prompt, out_prompt_size)) {
        layer->cache_hits++;
        layer->generation_count++;
...
continuation_prompt_error_t continuation_prompt_layer_get_performance(
    ...
    if (layer->generation_count > 0) {
        performance->avg_generation_time_ns = layer->total_time_ns / layer->generation_count;
        
        uint64_t total_lookups = layer->cache_hits + layer->cache_misses;
        if (total_lookups > 0) {
            performance->cache_hit_rate = (double)layer->cache_hits / (double)total_lookups;
        } else {
