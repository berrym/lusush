...
layer_events_error_t layer_events_init(layer_event_system_t *events) {
    ...
    if (events->config.max_queue_size == 0 || events->config.max_subscribers == 0) {
...
layer_events_error_t layer_events_publish(layer_event_system_t *events,
    ...
    if (!validate_event_system(events) || !event) {
        if (events) events->last_error = LAYER_EVENTS_ERROR_INVALID_PARAM;
        return LAYER_EVENTS_ERROR_INVALID_PARAM;
    }
    ...
    debug_log(events, "Publishing event type %d from layer %d", 
...
layer_events_error_t layer_events_subscribe(layer_event_system_t *events,
    ...
    debug_log(events, "Subscribing layer %d to event type %d", 
...
int layer_events_process_pending(layer_event_system_t *events,
    ...
    for (int priority = LAYER_EVENT_PRIORITY_CRITICAL; 
        ...
        while (true) {
            ...
            layer_event_t *event = dequeue_event(events, (layer_event_priority_t)priority);
            if (!event) {
            ...
            if (events->config.enable_performance_monitoring) {
                ...
                if (processing_time_ms > PERFORMANCE_WARNING_PROCESSING_TIME_MS) {
                    debug_log(events, "Performance warning: Event processing took %lu ms", 
                             processing_time_ms);
                }
            }
...
const char *layer_events_get_type_name(layer_event_type_t event_type) {
    switch (event_type) {
        case LAYER_EVENT_NONE: return "NONE";
        case LAYER_EVENT_CONTENT_CHANGED: return "CONTENT_CHANGED";
        case LAYER_EVENT_TEXT_UPDATED: return "TEXT_UPDATED";
        case LAYER_EVENT_STYLE_UPDATED: return "STYLE_UPDATED";
        case LAYER_EVENT_SIZE_CHANGED: return "SIZE_CHANGED";
        case LAYER_EVENT_POSITION_CHANGED: return "POSITION_CHANGED";
        case LAYER_EVENT_CURSOR_MOVED: return "CURSOR_MOVED";
        case LAYER_EVENT_REDRAW_NEEDED: return "REDRAW_NEEDED";
        case LAYER_EVENT_REFRESH_REQUESTED: return "REFRESH_REQUESTED";
        case LAYER_EVENT_VISIBILITY_CHANGED: return "VISIBILITY_CHANGED";
        case LAYER_EVENT_THEME_CHANGED: return "THEME_CHANGED";
        case LAYER_EVENT_COLOR_SCHEME_UPDATED: return "COLOR_SCHEME_UPDATED";
        case LAYER_EVENT_FONT_CHANGED: return "FONT_CHANGED";
        case LAYER_EVENT_INPUT_RECEIVED: return "INPUT_RECEIVED";
        case LAYER_EVENT_FOCUS_GAINED: return "FOCUS_GAINED";
        case LAYER_EVENT_FOCUS_LOST: return "FOCUS_LOST";
        case LAYER_EVENT_TERMINAL_RESIZE: return "TERMINAL_RESIZE";
        case LAYER_EVENT_INITIALIZATION_COMPLETE: return "INITIALIZATION_COMPLETE";
        case LAYER_EVENT_CLEANUP_REQUESTED: return "CLEANUP_REQUESTED";
        case LAYER_EVENT_ERROR_OCCURRED: return "ERROR_OCCURRED";
        case LAYER_EVENT_PERFORMANCE_WARNING: return "PERFORMANCE_WARNING";
        case LAYER_EVENT_DEBUG_INFO: return "DEBUG_INFO";
...
const char *layer_events_get_layer_name(layer_id_t layer_id) {
    switch (layer_id) {
        case LAYER_ID_UNKNOWN: return "UNKNOWN";
        case LAYER_ID_BASE_TERMINAL: return "BASE_TERMINAL";
        case LAYER_ID_TERMINAL_CONTROL: return "TERMINAL_CONTROL";
        case LAYER_ID_PROMPT_LAYER: return "PROMPT_LAYER";
        case LAYER_ID_COMMAND_LAYER: return "COMMAND_LAYER";
        case LAYER_ID_COMPOSITION_ENGINE: return "COMPOSITION_ENGINE";
        case LAYER_ID_DISPLAY_CONTROLLER: return "DISPLAY_CONTROLLER";
        case LAYER_ID_EXTERNAL: return "EXTERNAL";
...
static layer_events_error_t notify_subscribers(layer_event_system_t *events, 
    ...
    while (subscriber) {
        if (subscriber->subscriber.event_type == event->type &&
            ...
            layer_events_error_t result = subscriber->subscriber.callback(event, 
