...
static token_type_t classify_word(const char *text, size_t length, bool enable_keywords);
...
const char *token_type_name(token_type_t type) {
    switch (type) {
        ...
        case TOK_EXPANDABLE_STRING: return "EXPANDABLE_STRING";
        ...
        case TOK_REDIRECT_IN: return "REDIRECT_IN";
        case TOK_REDIRECT_OUT: return "REDIRECT_OUT";
        case TOK_APPEND: return "APPEND";
        case TOK_HEREDOC: return "HEREDOC";
        case TOK_HEREDOC_STRIP: return "HEREDOC_STRIP";
        case TOK_HERESTRING: return "HERESTRING";
        case TOK_REDIRECT_ERR: return "REDIRECT_ERR";
        case TOK_REDIRECT_BOTH: return "REDIRECT_BOTH";
        case TOK_APPEND_ERR: return "APPEND_ERR";
        case TOK_REDIRECT_FD: return "REDIRECT_FD";
        ...
        case TOK_GLOB: return "GLOB";
        case TOK_QUESTION: return "QUESTION";
        case TOK_COMMAND_SUB: return "COMMAND_SUB";
        case TOK_ARITH_EXP: return "ARITH_EXP";
        case TOK_BACKQUOTE: return "BACKQUOTE";
...
static token_t *tokenize_next(tokenizer_t *tokenizer) {
    ...
    if (tokenizer->position >= tokenizer->input_length) {
        return token_new(TOK_EOF, NULL, 0, tokenizer->line, 
                        tokenizer->column, tokenizer->position);
    }
    ...
    if (c == '\'' || c == '"') {
        ...
        return token_new(TOK_ERROR, &tokenizer->input[start_pos], 
                        tokenizer->position - start_pos, start_line, start_column, start_pos);
    }
    ...
    if (c == '$') {
        ...
        if (tokenizer->position < tokenizer->input_length) {
            ...
            if (next == '(') {
                ...
                if (tokenizer->position < tokenizer->input_length && 
                    ...
                    size_t length = tokenizer->position - start;
                    return token_new(TOK_ARITH_EXP, &tokenizer->input[start], length,
                                   start_line, start_column, start_pos);
                } else {
                    ...
                    size_t length = tokenizer->position - start;
                    return token_new(TOK_COMMAND_SUB, &tokenizer->input[start], length,
                                   start_line, start_column, start_pos);
                }
            } else if (next == '{') {
        ...
        size_t length = tokenizer->position - start;
        return token_new(TOK_WORD, &tokenizer->input[start], length,
                        start_line, start_column, start_pos);
    }
    ...
    if (is_operator_char(c)) {
        switch (c) {
            case ';':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_SEMICOLON, ";", 1, start_line, start_column, start_pos);
            ...
            case '|':
                if (tokenizer->position + 1 < tokenizer->input_length && 
                    ...
                    tokenizer->column += 2;
                    return token_new(TOK_LOGICAL_OR, "||", 2, start_line, start_column, start_pos);
                } else {
                    tokenizer->position++;
                    tokenizer->column++;
                    return token_new(TOK_PIPE, "|", 1, start_line, start_column, start_pos);
                }
            ...
            case '&':
                if (tokenizer->position + 1 < tokenizer->input_length) {
                    ...
                    if (next == '&') {
                        tokenizer->position += 2;
                        tokenizer->column += 2;
                        return token_new(TOK_LOGICAL_AND, "&&", 2, start_line, start_column, start_pos);
                    } else if (next == '>') {
                        tokenizer->position += 2;
                        tokenizer->column += 2;
                        return token_new(TOK_REDIRECT_BOTH, "&>", 2, start_line, start_column, start_pos);
                    }
                }
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_AND, "&", 1, start_line, start_column, start_pos);
                ...
                if (tokenizer->position + 1 < tokenizer->input_length) {
                    ...
                    if (next == '<') {
                        if (tokenizer->position + 2 < tokenizer->input_length && 
                            ...
                            tokenizer->column += 3;
                            return token_new(TOK_HERESTRING, "<<<", 3, start_line, start_column, start_pos);
                        } else if (tokenizer->position + 2 < tokenizer->input_length && 
                                  tokenizer->input[tokenizer->position + 2] == '-') {
                            tokenizer->position += 3;
                            tokenizer->column += 3;
                            return token_new(TOK_HEREDOC_STRIP, "<<-", 3, start_line, start_column, start_pos);
                        } else {
                            tokenizer->position += 2;
                            tokenizer->column += 2;
                            return token_new(TOK_HEREDOC, "<<", 2, start_line, start_column, start_pos);
                        }
                    }
                }
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_REDIRECT_IN, "<", 1, start_line, start_column, start_pos);
            ...
            case '>':
                if (tokenizer->position + 1 < tokenizer->input_length && 
                    ...
                    tokenizer->position += 2;
                    tokenizer->column += 2;
                    return token_new(TOK_APPEND, ">>", 2, start_line, start_column, start_pos);
                } else {
                    tokenizer->position++;
                    tokenizer->column++;
                    return token_new(TOK_REDIRECT_OUT, ">", 1, start_line, start_column, start_pos);
                }
                
            case '=':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_ASSIGN, "=", 1, start_line, start_column, start_pos);
                
            case '+':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_PLUS, "+", 1, start_line, start_column, start_pos);
                
            case '-':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_MINUS, "-", 1, start_line, start_column, start_pos);
            ...
            case '%':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_MODULO, "%", 1, start_line, start_column, start_pos);
                
            case '?':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_QUESTION, "?", 1, start_line, start_column, start_pos);
                
            case '(':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_LPAREN, "(", 1, start_line, start_column, start_pos);
                
            case ')':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_RPAREN, ")", 1, start_line, start_column, start_pos);
                
            case '{':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_LBRACE, "{", 1, start_line, start_column, start_pos);
                
            case '}':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_RBRACE, "}", 1, start_line, start_column, start_pos);
                
            case '[':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_LBRACKET, "[", 1, start_line, start_column, start_pos);
                
            case ']':
                tokenizer->position++;
                tokenizer->column++;
                return token_new(TOK_RBRACKET, "]", 1, start_line, start_column, start_pos);
        }
    }
    ...
    if (isalnum(c) || is_word_char(c)) {
        ...
        while (tokenizer->position < tokenizer->input_length) {
            ...
            if (is_word_char(curr) || isalnum(curr)) {
                if (!isdigit(curr)) is_numeric = false;
                tokenizer->position++;
                tokenizer->column++;
            } else {
                break;
        ...
        size_t length = tokenizer->position - start;
        token_type_t type = is_numeric ? TOK_NUMBER : 
