...
static bool is_keyword_at_position(const char *text, size_t pos, size_t text_len,
                                   const keyword_definition_t *keyword, bool *at_start);
...
static lle_construct_type_t determine_construct_type(lle_keyword_type_t first_keyword);

lle_result_t lle_structure_analyzer_get_default_config(
    lle_analyzer_config_t *config)
{
    if (!config) {
...
lle_result_t lle_structure_analyzer_create(
...
lle_result_t lle_structure_analyzer_destroy(
    lle_structure_analyzer_t *analyzer)
{
    if (!analyzer) {
        return LLE_ERROR_INVALID_PARAMETER;
...
lle_result_t lle_structure_analyzer_reset(
    lle_structure_analyzer_t *analyzer)
{
...
lle_result_t lle_structure_analyzer_detect_keywords(
    ...
    lle_keyword_match_t **keywords,
    size_t *keyword_count)
{
    if (!analyzer || !analyzer->active || !command_text || !keywords || !keyword_count) {
        return LLE_ERROR_INVALID_PARAMETER;
    }
    ...
    for (size_t i = 0; i < command_length; i++) {
        ...
        if (analyzer->context.in_quoted_string || analyzer->context.in_comment) {
            continue;
        }
        
        /* Check each keyword definition */
        for (const keyword_definition_t *kw = SHELL_KEYWORDS; kw->text != NULL; kw++) {
            bool at_start = false;
            if (is_keyword_at_position(command_text, i, command_length, kw, &at_start)) {
        ...
        for (const keyword_definition_t *kw = SHELL_KEYWORDS; kw->text != NULL; kw++) {
            ...
            if (is_keyword_at_position(command_text, i, command_length, kw, &at_start)) {
                if (!kw->requires_command_start || at_start) {
                    ...
                    kw_array[kw_idx].line_number = analyzer->context.current_line;
                    kw_array[kw_idx].is_command_start = at_start;
                    memset(kw_array[kw_idx].reserved, 0, sizeof(kw_array[kw_idx].reserved));
...
lle_result_t lle_structure_analyzer_detect_type(
    ...
    const char *command_text,
    size_t command_length,
    lle_construct_type_t *construct_type)
{
...
lle_result_t lle_structure_analyzer_check_complete(
    lle_structure_analyzer_t *analyzer,
    const char *command_text,
    size_t command_length,
    bool *is_complete,
    lle_keyword_type_t *missing_keyword)
{
    ...
    for (size_t i = 0; i < keyword_count; i++) {
        ...
        for (const keyword_pair_t *pair = pairs; pair->opening != LLE_KEYWORD_NONE; pair++) {
    ...
    *is_complete = (stack_size == 0);
    if (missing_keyword) {
        *missing_keyword = (stack_size > 0) ? stack[stack_size - 1] : LLE_KEYWORD_NONE;
    }
...
lle_result_t lle_structure_analyzer_calculate_indentation(
    ...
    const char *command_text,
    size_t command_length,
    lle_indentation_info_t **indent_info)
{
...
lle_result_t lle_structure_analyzer_analyze(
    lle_structure_analyzer_t *analyzer,
    const char *command_text,
    size_t command_length,
    lle_command_structure_t **structure)
{
    ...
    lle_result_t result = lle_command_structure_create(structure, analyzer->memory_pool);
    if (result != LLE_SUCCESS) {
        return result;
    }
    ...
    for (size_t i = 0; i < keyword_count; i++) {
        result = lle_command_structure_add_keyword(
            *structure,
            keywords[i].type,
            keywords[i].start_offset,
            keywords[i].line_number,
            0  /* Indent level calculated separately */
        );
                                         ...
                                         lle_command_structure_t *structure)
{
    ...
    /* For now, we have the flat keyword list which is sufficient for basic analysis */
