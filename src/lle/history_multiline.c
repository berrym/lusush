 ...
 */
static lle_result_t flatten_command(
    const char *original,
    char *flattened,
    ...
    lle_history_multiline_info_t *info)
{
    ...
    info->has_unclosed_quotes = (state.in_single_quote || 
                                  state.in_double_quote || 
                                  state.in_backtick);
    info->has_unclosed_brackets = (state.paren_count > 0 || 
                                    state.brace_count > 0 || 
                                    state.bracket_count > 0);
    info->is_function_def = state.in_function_definition;
    info->is_control_structure = (state.in_if_statement || 
                                   state.in_while_loop || 
                                   state.in_for_loop || 
                                   state.in_until_loop || 
    ...
    const char *original_multiline)
{
    ...
    lle_result_t result = lle_history_detect_multiline(original_multiline, &is_multiline);
    ...
    size_t *line_count)
{
    ...
    while (*start && index < count) {
        ...
        line_array[index].line_text = start;  /* Note: Not duplicating, just pointing */
