 ...
 */
static lle_result_t custom_generate_wrapper(
    ...
    for (size_t i = 0; i < g_custom_registry.count; i++) {
        ...
        if (entry->source.is_applicable) {
            if (!entry->source.is_applicable(entry->source.user_data, context)) {
                continue;
            }
        }
        
        /* Generate completions from this source */
        lle_result_t res = entry->source.generate(
            entry->source.user_data, context, prefix, result);
...
lle_result_t lle_completion_register_source(
    ...
    /* Create entry */
    custom_source_entry_t *entry = &g_custom_registry.entries[g_custom_registry.count];
...
bool lle_completion_source_is_custom(size_t index) {
    ...
    bool is_custom = 
        g_custom_registry.source_manager->sources[index]->type == LLE_SOURCE_CUSTOM;
    pthread_mutex_unlock(&g_custom_registry.mutex);
    return is_custom;
}
...
const char *lle_completion_get_custom_source_description(size_t index) {
    ...
    for (size_t i = 0; i < g_custom_registry.count; i++) {
        if (g_custom_registry.entries[i].registered) {
            if (current == index) {
                const char *desc = g_custom_registry.entries[i].description_copy;
                pthread_mutex_unlock(&g_custom_registry.mutex);
                return desc;
            }
            current++;
