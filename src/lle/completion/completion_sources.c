...
lle_result_t lle_completion_source_builtins(
...
lle_result_t lle_completion_source_aliases(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
...
lle_result_t lle_completion_source_commands(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
...
lle_result_t lle_completion_source_files(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
    ...
    while ((entry = readdir(d)) != NULL) {
        ...
        if (last_slash && dir_copy) {
            ...
            if (completion_text) {
                snprintf(completion_text, text_size, "%s/%s", dir_copy, entry->d_name);
            }
        ...
        lle_result_t res = lle_completion_result_add(
            result,
            completion_text ? completion_text : entry->d_name,
            suffix,
            type,
            score
        );
...
lle_result_t lle_completion_source_variables(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
    ...
    for (size_t i = 0; i < sizeof(special_vars) / sizeof(special_vars[0]); i++) {
...
lle_result_t lle_completion_source_history(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
