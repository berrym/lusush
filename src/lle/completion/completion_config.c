 ...
 */
static lle_result_t execute_command(const char *command, 
                                    char ***out_lines, 
                                    size_t *out_count) {
 ...
 */
static bool pattern_matches(const char *pattern, 
...
static bool config_source_is_applicable(void *user_data,
    ...
    lle_command_source_config_t *config = (lle_command_source_config_t *)user_data;
    ...
    for (size_t i = 0; i < config->applies_to_count; i++) {
        if (pattern_matches(config->applies_to[i], 
...
static lle_result_t config_source_generate(void *user_data,
    ...
    bool use_cache = config->cache_seconds > 0 && 
    ...
    if (use_cache) {
        ...
        /* Execute command */
        lle_result_t res = execute_command(config->command, &lines, &line_count);
        if (res != LLE_SUCCESS) {
    ...
    for (size_t i = 0; i < line_count; i++) {
        ...
        /* Add to results */
        lle_completion_add_item(result, 
                               lines[i],
                               config->suffix ? config->suffix : " ",
                               config->description,
                               700); /* Medium-high priority */
    }
 ...
 */
static lle_command_source_config_t *get_or_create_source(
    config_parser_ctx_t *ctx, const char *name) {
    ...
    if (config->source_count >= config->source_capacity) {
        size_t new_capacity = config->source_capacity == 0 ? 8 : 
                              config->source_capacity * 2;
        ...
        lle_command_source_config_t *new_sources = realloc(
            config->sources, 
    ...
    lle_command_source_config_t *source = &config->sources[config->source_count];
 ...
 */
static lle_result_t config_parser_callback(const char *section, 
                                           const char *key,
    ...
    lle_command_source_config_t *source = get_or_create_source(ctx, source_name);
    if (!source) {
...
lle_result_t lle_completion_load_config_file(const char *path) {
    ...
    if (g_completion_config.initialized) {
        ...
        memset(&g_completion_config.config, 0, sizeof(g_completion_config.config));
    }
    ...
    /* Parse config */
    config_parser_ctx_t ctx = {
        .config = &g_completion_config.config,
    ...
    for (size_t i = 0; i < g_completion_config.config.source_count; i++) {
        lle_command_source_config_t *src = &g_completion_config.config.sources[i];
