...
lle_result_t lle_completion_extract_word(
    const char *buffer,
    size_t cursor_pos,
    size_t *word_start,
    char **word)
...
lle_result_t lle_completion_analyze_context(
    const char *buffer,
    size_t cursor_pos,
    lle_completion_context_info_t *context)
{
    ...
    size_t word_start = 0;
    char *word = NULL;
    lle_result_t result = lle_completion_extract_word(buffer, cursor_pos, 
                                                       &word_start, &word);
...
lle_result_t lle_completion_generate_commands(
...
lle_result_t lle_completion_generate_arguments(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
...
lle_result_t lle_completion_generate_variables(
    lle_memory_pool_t *memory_pool,
    const char *prefix,
    lle_completion_result_t *result)
{
    ...
    lle_completion_result_t **result)
{
    ...
    // Analyze context
    lle_completion_context_info_t context;
    lle_result_t res = lle_completion_analyze_context(buffer, cursor_pos, &context);
    if (res != LLE_SUCCESS) {
        return res;
    }
    ...
    switch (context.type) {
        case LLE_COMPLETION_CONTEXT_COMMAND:
            res = lle_completion_generate_commands(memory_pool, prefix, comp_result);
            break;
            
        case LLE_COMPLETION_CONTEXT_VARIABLE:
            res = lle_completion_generate_variables(memory_pool, prefix, comp_result);
            break;
            
        case LLE_COMPLETION_CONTEXT_ARGUMENT:
            res = lle_completion_generate_arguments(memory_pool, prefix, comp_result);
            break;
