...
static size_t calculate_indented_size(const char *text, size_t length,
                                      lle_command_structure_t *structure,
                                      const lle_reconstruction_options_t *options);
static lle_result_t apply_indentation_to_lines(lle_reconstruction_engine_t *engine,
    ...
    const lle_reconstruction_options_t *options)
{
    ...
    if (options) {
        memcpy(&new_engine->options, options, sizeof(lle_reconstruction_options_t));
    } else {
...
{
    if (!engine || !engine->active || !structure || 
        !original_text || !indented_text || !indented_length) {
        return LLE_ERROR_INVALID_PARAMETER;
    }
    ...
    size_t output_size = calculate_indented_size(
        original_text, original_length, structure, &engine->options);
    ...
    size_t out_len = 0;
    res = apply_indentation_to_lines(
        engine,
                                      ...
                                      const lle_reconstruction_options_t *options)
{
    ...
    return length + (max_indent * line_count) + line_count;  /* +line_count for newlines */
}
...
static size_t get_indent_for_line(lle_command_structure_t *structure,
                                   ...
                                   const lle_reconstruction_options_t *options)
{
    if (!structure->indentation || line_number >= structure->indentation->line_count) {
        return 0;
    }
    ...
    size_t indent_level = indent_spaces / (structure->indentation->spaces_per_level > 0 ? 
                                           structure->indentation->spaces_per_level : 2);
                                                ...
                                                size_t *output_length)
{
    ...
    while (current_line) {
        ...
        if (content_len > 0) {
            memcpy(&output[out_pos], &current_line->content[content_start], content_len);
            out_pos += content_len;
        }
