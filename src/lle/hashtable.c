...
static void lle_set_current_memory_context(lle_hashtable_memory_context_t *ctx) {
...
lle_result_t lle_hashtable_config_create_pooled(
...
lle_result_t lle_hashtable_registry_add(lle_hashtable_registry_t *registry,
    ...
    if (registry->count >= registry->capacity) {
        ...
        lle_strstr_hashtable_t **new_hashtables = realloc(
            registry->hashtables,
...
lle_result_t lle_hashtable_factory_create_strstr(
    ...
    const lle_hashtable_config_t *cfg = config ? config : factory->default_config;
    
    /* Allocate wrapper structure */
    lle_strstr_hashtable_t *ht_wrapper = calloc(1, sizeof(lle_strstr_hashtable_t));
    if (!ht_wrapper) {
        return LLE_ERROR_OUT_OF_MEMORY;
    }
    ...
    if (cfg->performance_monitoring) {
        ht_wrapper->metrics = calloc(1, sizeof(lle_hashtable_performance_metrics_t));
...
lle_result_t lle_hashtable_factory_create_generic(
    ...
    /* Allocate wrapper structure */
    lle_generic_hashtable_t *ht_wrapper = calloc(1, sizeof(lle_generic_hashtable_t));
    if (!ht_wrapper) {
        return LLE_ERROR_OUT_OF_MEMORY;
    }
 ...
 * libhashtable's internal linked list implementation for hash collision handling
 * is not inherently thread-safe. Under high concurrency with many hash collisions,
 * some entries may be lost even with external locking (typically 90-95% success rate).
 * 
 * This is a known limitation of libhashtable's architecture. For scenarios requiring
 * 100% reliability under concurrent writes, consider:
        ...
        if (keys) {
            while (ht_strstr_enum_next(enumerator, &key, &value)) {
                if (key_count >= key_capacity) {
                    key_capacity *= 2;
                    char **new_keys = realloc(keys, key_capacity * sizeof(char*));
                    if (!new_keys) break;
                    keys = new_keys;
                }
...
lle_result_t lle_hashtable_get_metrics(lle_strstr_hashtable_t *ht,
...
lle_result_t lle_hashtable_system_init(lle_hashtable_system_t **system,
    ...
    /* Initialize factory */
    lle_result_t result = lle_hashtable_factory_init(&sys->factory, memory_pool);
    if (result != LLE_SUCCESS) {
        free(sys);
        return result;
    }
