...
static lle_result_t insert_timer_sorted(lle_timer_system_t *ts, lle_timer_event_t *timer) {
    /* Grow array if needed */
    if (ts->timer_count >= ts->timer_capacity) {
        size_t new_capacity = ts->timer_capacity * 2;
        lle_timer_event_t **new_array = realloc(ts->timers, 
                                                 new_capacity * sizeof(lle_timer_event_t *));
        if (!new_array) {
    ...
    if (insert_pos < ts->timer_count) {
        memmove(&ts->timers[insert_pos + 1],
                &ts->timers[insert_pos],
    ...
    while (i < ts->timer_count) {
        ...
        if (event_to_dispatch) {
            ...
            if (timer->event->data && timer->event->data_size > 0) {
                ...
                if (event_to_dispatch->data) {
                    memcpy(event_to_dispatch->data, timer->event->data, timer->event->data_size);
                } else {
        ...
        if (timer->repeating) {
            ...
            if (i < ts->timer_count - 1) {
                memmove(&ts->timers[i],
                        &ts->timers[i + 1],
                        (ts->timer_count - i - 1) * sizeof(lle_timer_event_t *));
            }
            ts->timer_count--;
