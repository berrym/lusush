...
lle_result_t lle_event_enhanced_stats_init(lle_event_system_t *system) {
    ...
    lle_event_enhanced_stats_t *stats = lle_pool_alloc(sizeof(lle_event_enhanced_stats_t));
    if (!stats) {
    ...
    stats->type_stats = lle_pool_alloc(sizeof(lle_event_type_stats_t) * stats->type_stats_capacity);
    if (!stats->type_stats) {
        lle_pool_free(stats);
        return LLE_ERROR_OUT_OF_MEMORY;
    }
    
    memset(stats->type_stats, 0, sizeof(lle_event_type_stats_t) * stats->type_stats_capacity);
...
lle_result_t lle_event_enhanced_stats_get_type(lle_event_system_t *system,
    ...
    for (size_t i = 0; i < stats->type_stats_count; i++) {
        if (stats->type_stats[i].event_type == type) {
            memcpy(stats_out, &stats->type_stats[i], sizeof(lle_event_type_stats_t));
...
lle_result_t lle_event_enhanced_stats_get_cycles(lle_event_system_t *system,
    ...
    if (min_time) {
        *min_time = (stats->min_cycle_time == UINT64_MAX) ? 0 : stats->min_cycle_time;
    }
    if (max_time) {
