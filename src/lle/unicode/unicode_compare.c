 ...
 * Focus on Latin-1 Supplement (U+0080-U+00FF) and Latin Extended-A (U+0100-U+017F)
 * which covers most European languages.
 * ============================================================================ */
 ...
 */
static bool find_composition(uint32_t base, uint32_t combining, uint32_t *composed) {
...
int lle_unicode_decompose(uint32_t codepoint, uint32_t *decomposed, int max_length) {
...
int lle_unicode_normalize_nfc(const char *input, size_t input_len,
    ...
    while (ptr < end && decomposed_count < MAX_DECOMPOSED - 4) {
        ...
        if (parts_count > 0) {
            /* Has decomposition */
            for (int i = 0; i < parts_count && decomposed_count < MAX_DECOMPOSED; i++) {
    ...
    for (size_t i = 1; i < decomposed_count; i++) {
        ...
        if (ccc == 0) continue;  /* Starter, don't reorder */
    ...
    for (size_t i = 0; i < decomposed_count; i++) {
        ...
        while (last_starter > 0 && lle_unicode_combining_class(composed[last_starter]) != 0) {
            last_starter--;
        }
...
bool lle_unicode_strings_equal(const char *str1, const char *str2,
    ...
    return lle_unicode_strings_equal_n(str1, strlen(str1), 
                                       str2, strlen(str2), 
                                       options);
}
...
bool lle_unicode_strings_equal_n(const char *str1, size_t len1,
    ...
    lle_unicode_compare_options_t opts = options ? *options : LLE_UNICODE_COMPARE_DEFAULT;
    ...
    if (opts.trim_whitespace) {
        while (s1 < e1 && (*s1 == ' ' || *s1 == '\t')) s1++;
        while (e1 > s1 && (e1[-1] == ' ' || e1[-1] == '\t')) e1--;
        while (s2 < e2 && (*s2 == ' ' || *s2 == '\t')) s2++;
        while (e2 > s2 && (e2[-1] == ' ' || e2[-1] == '\t')) e2--;
    }
    ...
    if (opts.normalize) {
        ...
        if (lle_unicode_normalize_nfc(s1, trimmed_len1, norm1, NORM_BUF_SIZE, &norm1_len) != 0) {
            /* Normalization failed, fall back to byte comparison */
            goto byte_compare;
        }
        if (lle_unicode_normalize_nfc(s2, trimmed_len2, norm2, NORM_BUF_SIZE, &norm2_len) != 0) {
            goto byte_compare;
        }
        ...
        if (opts.case_insensitive) {
            ...
            if (norm1_len != norm2_len) return false;
...
byte_compare:
    ...
    if (trimmed_len1 != trimmed_len2) return false;
