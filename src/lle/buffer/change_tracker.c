 ...
 */
static lle_change_sequence_t *find_last_undoable_sequence(lle_change_tracker_t *tracker) {
 ...
 */
static void free_operation(lle_change_operation_t *op, lusush_memory_pool_t *pool) {
    if (!op) {
        return;
    }
 ...
 */
static void free_sequence(lle_change_sequence_t *seq, lusush_memory_pool_t *pool) {
    if (!seq) {
        return;
    }
...
lle_result_t lle_change_tracker_init(lle_change_tracker_t **tracker,
    ...
    lle_change_tracker_t *t = (lle_change_tracker_t *)lle_pool_alloc(
    ...
    t->max_undo_levels = (max_undo_levels > 0) ? 
        max_undo_levels : LLE_BUFFER_MAX_UNDO_LEVELS;
...
lle_result_t lle_change_tracker_begin_sequence(lle_change_tracker_t *tracker,
    ...
    lle_change_sequence_t *seq = (lle_change_sequence_t *)lle_pool_alloc(
        sizeof(lle_change_sequence_t));
    if (!seq) {
        return LLE_ERROR_OUT_OF_MEMORY;
    }
