...
lle_result_t lle_keybinding_get_user_config_path(char *buffer,
    ...
    if (xdg_config && xdg_config[0]) {
        int written = snprintf(buffer, buffer_size, "%s/lusush/%s",
                               xdg_config, LLE_KEYBINDING_CONFIG_FILENAME);
    ...
    int written = snprintf(buffer, buffer_size, "%s/.config/lusush/%s",
                           home, LLE_KEYBINDING_CONFIG_FILENAME);
    if (written < 0 || (size_t)written >= buffer_size) {
 ...
 */
static lle_result_t keybinding_config_callback(
    const char *section,
    const char *key,
    ...
    if (strcmp(action_name, "none") == 0 || action_name[0] == '\0') {
        lle_result_t unbind_result = lle_keybinding_manager_unbind(
            ctx->manager, key_sequence);
    ...
    const lle_action_registry_entry_t *entry = lle_action_registry_lookup(action_name);
    if (!entry) {
    ...
    lle_result_t parse_result = lle_key_sequence_parse(key_sequence, &key_event);
    ...
    if (lle_keybinding_manager_lookup(ctx->manager, key_sequence, &existing) == LLE_SUCCESS) {
...
lle_result_t lle_keybinding_load_from_string(lle_keybinding_manager_t *manager,
    ...
    keybinding_parser_ctx_t ctx = {
        .manager = manager,
        .result = result
    };

    /* Parse and apply bindings */
    lle_result_t parse_result = lle_theme_parser_parse(
        &parser, keybinding_config_callback, &ctx);
    ...
    if (parse_result != LLE_SUCCESS) {
        ...
        snprintf(result->error_msg, sizeof(result->error_msg),
                 "%s", lle_theme_parser_error(&parser));
        return parse_result;
    }
...
lle_result_t lle_keybinding_load_from_file(lle_keybinding_manager_t *manager,
    ...
    /* Parse content */
    lle_result_t load_result = lle_keybinding_load_from_string(
        manager, content, result);
...
lle_result_t lle_keybinding_load_user_config(lle_keybinding_manager_t *manager,
    ...
    lle_result_t path_result = lle_keybinding_get_user_config_path(
