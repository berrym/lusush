...
lle_result_t lle_parser_state_machine_init(lle_parser_state_machine_t **state_machine,
    ...
    lle_parser_state_machine_t *sm = lle_pool_alloc(sizeof(lle_parser_state_machine_t));
    if (!sm) {
...
lle_result_t lle_parser_state_machine_process(lle_parser_state_machine_t *state_machine,
    ...
    switch (state_machine->current_state) {
        case LLE_PARSER_STATE_NORMAL:
            ...
            if (data[0] == '\x1B') {
                lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_ESCAPE);
            }
            ...
            if (data_len >= 2) {
                if (data[1] == '[') {
                    ...
                    if (data_len >= 3) {
                        if (data[2] == 'M' || data[2] == '<') {
                            /* Mouse sequence */
                            lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_MOUSE);
                        } else {
                            /* CSI sequence */
                            lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_CSI);
                        }
                    }
                } else if (data[1] == ']') {
                    /* OSC sequence */
                    lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_OSC);
                } else if (data[1] == 'P') {
                    /* DCS sequence */
                    lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_DCS);
                } else if (data[1] == 'O' || data[1] == 'N') {
                    /* SS3 or SS2 - key sequence */
                    lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_KEY_SEQUENCE);
                } else {
        ...
        case LLE_PARSER_STATE_ERROR_RECOVERY:
            ...
            lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_NORMAL);
            state_machine->error_recoveries++;
            break;
            
        default:
            /* Unknown state - recover */
            lle_parser_state_machine_transition(state_machine, LLE_PARSER_STATE_ERROR_RECOVERY);
            state_machine->error_recoveries++;
            break;
    }
