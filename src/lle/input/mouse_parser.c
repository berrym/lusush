...
static lle_result_t parse_x10_sequence(lle_mouse_parser_t *parser,
    ...
    if ((btn - 32) & 0x04) event->modifiers |= LLE_KEY_MOD_SHIFT;
    if ((btn - 32) & 0x08) event->modifiers |= LLE_KEY_MOD_ALT;
    if ((btn - 32) & 0x10) event->modifiers |= LLE_KEY_MOD_CTRL;
    ...
    if (is_wheel) {
        ...
        event->button = (button_code == 0) ? LLE_MOUSE_BUTTON_WHEEL_UP : LLE_MOUSE_BUTTON_WHEEL_DOWN;
        ...
        event->button = (lle_mouse_button_t)(button_code + 1);  // Convert 0-2 to 1-3
...
static lle_result_t parse_sgr_sequence(lle_mouse_parser_t *parser,
    ...
    // Decode modifiers
    event->modifiers = LLE_KEY_MOD_NONE;
    if (btn & 0x04) event->modifiers |= LLE_KEY_MOD_SHIFT;
    if (btn & 0x08) event->modifiers |= LLE_KEY_MOD_ALT;
    if (btn & 0x10) event->modifiers |= LLE_KEY_MOD_CTRL;
...
lle_result_t lle_mouse_parser_parse_sequence(lle_mouse_parser_t *parser,
    ...
    *event_info = NULL;
    ...
    lle_mouse_event_info_t *event = lle_pool_alloc(sizeof(lle_mouse_event_info_t));
    if (!event) {
        return LLE_ERROR_OUT_OF_MEMORY;
    }
    ...
    } else if (sequence_len >= 9 && sequence[0] == '\x1B' && sequence[1] == '[' && sequence[2] == '<') {
        // SGR format
