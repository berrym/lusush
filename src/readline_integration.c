 ...
 * clang-format will try to sort includes alphabetically, which breaks this.
 */
// clang-format off
#include <stdio.h>
...
static char **lusush_display_subcommand_completion(const char *text,
...
bool lusush_readline_init(void) {
    ...
    // SELECTIVE completion setup - enable TAB completion while preserving arrow
    // keys Critical: Only TAB should trigger completion, never arrow keys
    ...
    if (!history_file_path) {
        ...
        if (home) {
            ...
            if (history_file_path) {
                snprintf(history_file_path, path_len, "%s/.lusush_history",
                         home);
            }
        }
        if (!history_file_path) {
...
char *lusush_readline_with_prompt(const char *prompt) {
    ...
    // MINIMAL INTEGRATION: Only enhance primary prompts, preserve established
    // input.c flow
    char *line = NULL;
    ...
    if (config.use_lle) {
        ...
        // CRITICAL FIX: Always use standard readline to prevent history
        // corruption Layered display deviation was causing history access
        // isolation Keep single unified readline path for consistent history
        // behavior
        line = readline(actual_prompt);
    }

    // Note: Do not free themed_prompt here - it's managed by readline cleanup
    // system The themed_prompt becomes current_prompt and will be freed in
    ...
    // if (line && *line && config.enhanced_display_mode &&
...
bool lusush_readline_available(void) { return true; }

const char *lusush_readline_version(void) { return rl_library_version; }
...
static void add_to_history_cache(const char *line) {
    ...
    if (history_cache_size >= history_cache_capacity) {
        ...
        char **new_cache =
            realloc(history_cache, new_capacity * sizeof(char *));
        if (new_cache) {
...
void lusush_history_add(const char *line) {
    ...
    /* When LLE is enabled, it manages its own history - do not use GNU Readline
     * history */
    ...
    if (shell_opts.nolog) {
        // Simple detection for function definitions: look for pattern "name()"
        // or "name ()"
        ...
        if (paren_pos) {
            ...
            if (name_end > name_start) {
                ...
                // Check if the character before () is part of a valid
                // identifier
                if (isalnum(*p) || *p == '_') {
                    skip_history =
                        true; // Skip adding function definitions to history
                }
            }
        }
    }

    // Check for duplicates if hist_no_dups is enabled (skip for function
    // definitions when nolog is enabled)
    ...
    // Add to readline history (skip for function definitions when nolog is
    // enabled)
    if (!skip_history) {
        add_history(line);
    }
...
int lusush_history_length(void) { return history_length; }
...
static char **lusush_tab_completion(const char *text, int start,
    ...
    // display command - supports nested subcommands (display lle, display
    // performance)
        ...
        if (matches) {
            ...
            if (completions.len == 1) {
                ...
                for (size_t i = 1; i < completions.len; i++) {
                    size_t j = 0;
                    while (j < prefix_len && j < strlen(completions.cvec[i]) &&
    ...
    // Fallback: if no specific completion was handled, use readline's default
    // file completion
    if (matches == NULL) {
...
static char *lusush_completion_entry_function(const char *text, int state) {
    (void)text;
    (void)state; // Suppress unused parameter warnings
...
static char **lusush_git_subcommand_completion(const char *text) {
    ...
    static const char *git_commands[] = {
        ...
        "checkout",    "branch", "log",         "diff",         "merge",
        "fetch",       "clone",  "init",        "reset",        "rebase",
        "tag",         "config", "remote",      "show",         "stash",
        "cherry-pick", "revert", "bisect",      "grep",         "mv",
        "rm",          "clean",  "describe",    "shortlog",     "archive",
        "bundle",      "blame",  "show-branch", "format-patch", NULL};
 ...
 */
static char **lusush_config_subcommand_completion(const char *text) {
    static const char *config_subcommands[] = {"show",   "set",  "get",
                                               "reload", "save", NULL};
...
static char **lusush_display_subcommand_completion(const char *text,
    ...
    static const char *display_lle_subcommands[] = {
        ...
        "keybindings", "diagnostics", "autosuggestions",
        "syntax",      "multiline",   "history-import",
        NULL};
 ...
 */
static char **lusush_history_subcommand_completion(const char *text) {
    static const char *history_subcommands[] = {"show", "clear", "delete",
                                                "search", NULL};
...
char *lusush_generate_prompt(void) {
    ...
    // STABILITY FIX: Disable enhanced prompt in enhanced display mode to
    // prevent corruption Phase 1: Enable layered display integration for prompt
    // generation Use enhanced prompt when layered display is available
...
static int lusush_next_history(int count, int key) {
    ...
    // Check if we have content BEFORE calling rl_get_next_history (which might
    // clear it)
    ...
    if (next) {
        ...
        if (has_content) {
            ...
            // Let readline handle line clearing properly to avoid display
            // corruption
            rl_replace_line("", 0);
            rl_point = 0;
            rl_end = 0;
...
static void setup_key_bindings(void) {
    ...
    rl_bind_keyseq("\\e[1;5C",
                   rl_forward_word); // Ctrl+Right arrow: forward word

    // Set initial editing mode
    update_readline_editing_mode();
}

void lusush_keybindings_setup(void) { setup_key_bindings(); }
...
int lusush_keybinding_remove(int key) { return rl_unbind_key(key); }
...
static void setup_readline_config(void) {
    ...
    rl_catch_signals =
        0; // Let shell handle signals for child processes like git
    ...
    rl_variable_bind("disable-completion",
                     "off"); // MASTER SWITCH: Enable completion for TAB
    rl_variable_bind("show-all-if-unmodified",
                     "off"); // CRITICAL: Prevents arrow key completion
    rl_variable_bind("show-all-if-ambiguous",
                     "off"); // CRITICAL: Prevents "display all possibilities"
    rl_variable_bind("visible-stats", "off"); // Disable to prevent interference
    rl_variable_bind("page-completions",
                     "on"); // Enable paging for large completion lists
    rl_variable_bind("completion-query-items",
                     "50"); // Optimized threshold for large sets
    rl_variable_bind("completion-display-width",
                     "4"); // Optimize column display
    rl_variable_bind("print-completions-horizontally",
                     "on"); // Better layout for many items

    // Multi-line prompt handling for Ctrl+R positioning
    rl_variable_bind("horizontal-scroll-mode",
                     "off"); // Use vertical scrolling for long lines
...
void lusush_refresh_line(void) {
    ...
    if (display_integration_is_layered_active()) {
        if (debug_enabled) {
            fprintf(stderr, "[READLINE_DEBUG] lusush_refresh_line: calling "
...
int lusush_get_cursor_position(void) { return rl_point; }
...
const char *lusush_get_line_buffer(void) { return rl_line_buffer; }
...
void lusush_readline_set_debug(bool enabled) { debug_enabled = enabled; }
...
static char *lusush_readline_with_layered_display(const char *prompt) {
    ...
    if (!display_integration_is_layered_active()) {
        if (debug_enabled) {
            fprintf(stderr, "[READLINE_DEBUG] Layered display not active "
                            "during readline call\n");
        }
    ...
    if (generated_prompt) {
        ...
        strncpy(layered_prompt_buffer, generated_prompt,
                sizeof(layered_prompt_buffer) - 1);
        layered_prompt_buffer[sizeof(layered_prompt_buffer) - 1] = '\0';

        if (debug_enabled) {
            fprintf(stderr,
                    "[READLINE_DEBUG] Using generated layered prompt (length: "
                    "%zu)\n",
        ...
        if (debug_enabled) {
            fprintf(stderr, "[READLINE_DEBUG] Failed to generate layered "
                            "prompt, using fallback\n");
        }
        return NULL; // Fallback to standard readline
    }
}

bool lusush_readline_is_debug_enabled(void) { return debug_enabled; }
