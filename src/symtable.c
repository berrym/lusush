...
void init_symtable(void) {
    if (global_manager) return; // Already initialized
...
char **get_environ_array(void) {
    return symtable_get_environment_array();
}

void free_environ_array(char **env) {
    symtable_free_environment_array(env);
}
...
void symtable_benchmark_comparison(int iterations) {
    (void)iterations;
    printf("Optimized libhashtable implementation is the main implementation\n");
}
...
int symtable_pop_scope_opt_api(void) {
    return symtable_pop_scope_enhanced();
}
